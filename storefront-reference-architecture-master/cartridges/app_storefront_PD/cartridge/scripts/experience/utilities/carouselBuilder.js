...
function init(model, context) {
    ...
    var xsColSize =
        12 / parseInt(context.content.xsCarouselSlidesToDisplay, 10);
    var smColSize =
        12 / parseInt(context.content.smCarouselSlidesToDisplay, 10);
    var mdColSize =
        12 / parseInt(context.content.mdCarouselSlidesToDisplay, 10);

    var sizeExtraSmall = " col-" + xsColSize;
    var sizeSmall = " col-sm-" + smColSize;
    var sizeMedium = " col-md-" + mdColSize;

    model.regions.slides.setClassName("carousel-inner d-flex");
    model.regions.slides.setComponentClassName(
        "carousel-item" + sizeExtraSmall + sizeSmall + sizeMedium
    );
    model.regions.slides.setComponentClassName(
        "carousel-item active" + sizeExtraSmall + sizeSmall + sizeMedium,
        { position: 0 }
    );
    ...
    model.insufficientNumberOfSlides = {
        xs:
            context.content.xsCarouselSlidesToDisplay >= numberOfSlides
                ? "insufficient-xs-slides"
                : "",
        sm:
            context.content.smCarouselSlidesToDisplay >= numberOfSlides
                ? "insufficient-sm-slides"
                : "",
        md:
            context.content.mdCarouselSlidesToDisplay >= numberOfSlides
