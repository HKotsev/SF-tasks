...
server.post(
    ...
    function (req, res, next) {
        ...
        var pliUUID =
            req.form.productLineItemUUID || req.querystring.productLineItemUUID;
...
server.post(
    ...
    function (req, res, next) {
        ...
        var shippingFormErrors = COHelpers.validateShippingForm(
        ...
        if (Object.keys(shippingFormErrors).length > 0) {
            ...
            result.giftMessage = result.isGift
                ? form.shippingAddress.giftMessage.value
                : null;
        ...
        this.on("route:BeforeComplete", function (req, res) {
                ...
                try {
                    Transaction.wrap(function () {
                        if (origUUID === shipmentUUID) {
                            ...
                            var productLineItem = COHelpers.getProductLineItem(
                                basket,
                                pliUUID
                            );
                            if (shipmentUUID === "new") {
                                ...
                                if (
                                    origUUID === basket.defaultShipment.UUID &&
                                    basket.defaultShipment.productLineItems
                                        .length === 1
                                ) {
                                    ...
                                    shipment = basket.createShipment(
            ...
            for (var i = 0, ii = basket.shipments.length; i < ii; i++) {
                isValid = req.session.privacyCache.get(
                    basket.shipments[i].UUID
                );
                if (isValid !== "valid") {
                    allValid = false;
                    break;
            ...
            if (!basket.billingAddress) {
                if (
                    req.currentCustomer.addressBook &&
